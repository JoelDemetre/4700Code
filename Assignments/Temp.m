...
for i = 0:Timestep:endtime
   % pause(.1);
    ...
    for kt = 1:NumParticles
       ...
       %% B) Specular or Diffusive
       % The Specular or Diffusive Boundaries are based upon a probability
       % statement when they hit the boundaries namely DiffusionBarrierProbability
           ...
           if rand > DiffusionBarrierProbability
            if xinter2 || xinter1
             ...
             if yinter2 || yinter1
               ...
               temp = normrnd(vth, sqrt(mn/(Kbolt*T)), 1, NumParticles);
               if xinter1
                   x(3,kt) = pi/2 + rand*pi;
               elseif xinter2
                   x(3,kt) = pi/2 - rand*pi;
